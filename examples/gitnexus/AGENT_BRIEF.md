...
- Treat `syntheticCapture` as the most-called symbol (53 distinct callers per the call-graph resolver) — signature changes touch every caller `[INFERRED]`
- When you touch `gitnexus/src/core/ingestion/parsing-processor.ts`, also check `gitnexus/src/core/ingestion/workers/parse-worker.ts` — they co-change in 56 shared commit(s) per the commit co-change join `[INFERRED]`
