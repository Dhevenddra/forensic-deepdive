...
> v0.10 **"The Upgrade Path"** — an integrity release. v0.9 shipped correct and then
> the findings run caught `--refresh-shims` being structurally unable to refresh half
> its targets, with 909 tests green over it. The cause was not the predicate: **every
> test in the suite wrote shims into an empty directory, so only the *first* run had
> ever been tested.** Everyone who is not a new user lives on the untested path.
>
> So this release adds almost no surface. It makes the guarantees we already made
> real: that running Deepdive over a repo which already carries a previous release's
> output converges, that it says so when it has work to do, and that a stale
> `examples/` fails CI instead of being spotted by eye mid-release.
>
> It also contains a **cancelled feature**. The planned performance work targeted
> PageRank; the profile that was required before touching it measured PageRank at
> **0.04 %** of a large extract and the already-batched store writes at **79–87 %**.
> The optimization was disproven before it was written. That is the gate working.
>
> Engine, graph, the 5-artifact contract and the 9 MCP tools are **unchanged**.
> Emitted content is **byte-identical** except the version footer.

### Added
- **`forensic extract --timings`** — per-phase wall clock with sub-steps, slowest first.
  The instrumentation is always on (two `perf_counter` calls per phase); the flag only
  prints it. Sub-steps that don't account for their phase show an `other` row, and ones
  that *overlap* are reported as `OVERLAP` rather than silently hidden.
- **A stale-shim advisory on `extract`** — when generated shims are out of date, one line
  names the count and `--refresh-shims`. Detection only: it never touches the disk, is
  silent on a first run, and never counts a hand-edited file (that's yours, not stale).
- **`scripts/regen_examples.py`** — the `examples/` regeneration script, previously
  unversioned on a single machine while releases depended on it. Test-repo root comes
  from `$FORENSIC_EXAMPLES_SRC`; exit codes distinguish "an extract failed" (1) from
  "hygiene problem" (2) from "usage error" (3), where the old one conflated all three.

### Changed
- **Shim summary rows name the skill.** All five emitted skills are called `SKILL.md`, so
  refreshing two of them printed `Refreshed  SKILL.md, SKILL.md`. Now
  `codebase-refactoring/SKILL.md`. The written row had the same defect, five times over.
- **Track B was retargeted mid-cycle** (DEC-117): the profile cancelled the PageRank
  optimization outright rather than deferring it, promoted incremental extract, and
...
### Testing
- **`tests/test_upgrade_path.py`** — the suite's first tests that run over a **non-empty**
  repo. `tests/fixtures/prior_release/v0_8_0/` is the verbatim surface 0.8.0 wrote,
  generated by executing `emit/shims.py` at the `v0.8.0` tag — historical evidence, not a
  mock, and it reproduces the exact leak seen in the wild. Asserts **convergence** (an
  upgraded repo is byte-identical to a fresh one) *and* **restraint** (hand-edited and
  foreign files untouched), plus a vacuity guard and a test that monkeypatches the old
  predicate back in to prove the guarantee can still fail.
- **`tests/test_release_hygiene.py`** — `examples/` footers must match `pyproject.toml`,
  briefs stay under the 5 KB cap, and no internal ledger IDs ship. This deliberately makes
  the suite red between a version bump and the `examples/` regeneration.
- **`tests/test_timings.py`** — including the disjointness invariant that caught a nested
  sub-step double-counting the profile while it was being written.
